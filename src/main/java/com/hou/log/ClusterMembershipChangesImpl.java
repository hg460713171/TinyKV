...
import com.hou.node.NodeStatus;
import com.hou.node.PeerNode;
import com.hou.rpc.Request;
...
public class ClusterMembershipChangesImpl implements ClusterMembershipChanges {
     ...
     */
    @Override
    public synchronized Result addPeer(PeerNode newPeer) {
        // 已经存在
        ...
        if (node.status == NodeStatus.LEADER) {
            ...
            for (PeerNode ignore : node.peerSet.getPeersWithOutSelf()) {
                // TODO 同步到其他节点.
     ...
     */
    @Override
    public synchronized Result removePeer(PeerNode oldPeer) {
