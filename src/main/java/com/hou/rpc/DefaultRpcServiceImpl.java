...
import com.hou.log.AentryParam;
import com.hou.log.ClientKVReq;
import com.hou.log.ClusterMembershipChanges;
import com.hou.log.RvoteParam;
import com.hou.node.DefaultNode;
import com.hou.node.PeerNode;
...
public class DefaultRpcServiceImpl implements RpcService {
    ...
    public Response<?> handlerRequest(Request request) {
        if (request.getCmd() == Request.R_VOTE) {
            ...
            return new Response<>(((ClusterMembershipChanges) node).removePeer((PeerNode) request.getObj()));
        } else if (request.getCmd() == Request.CHANGE_CONFIG_ADD) {
            return new Response<>(((ClusterMembershipChanges) node).addPeer((PeerNode) request.getObj()));
        }
        return null;
    }
