...
import com.hou.common.LongConvert;
import com.hou.consensus.Consensus;
import com.hou.consensus.DefaultConsensus;
import com.hou.exception.RaftRemotingException;
import com.hou.log.*;
import com.hou.rpc.*;
import com.hou.stateMachine.StateMachine;
import com.hou.threadPool.RaftThreadPool;
...
public class DefaultNode implements Node, ClusterMembershipChanges {
    ...
    public Map<PeerNode, Long> nextIndexs;

    /** 对于每一个服务器，已经复制给他的日志的最高索引值 */
    public Map<PeerNode, Long> matchIndexs;
    ...
    public void setConfig(NodeInfo config) {
        ...
        for (String s : config.getPeerNodes()) {
            PeerNode peer = new PeerNode(s);
            peerSet.addPeer(peer);
            if (s.equals("localhost:" + config.getSelfNode())) {
                peerSet.setSelf(peer);
            }
        }

        rpcServer = new DefaultRpcServiceImpl(config.getSelfNode(), this);
    }
    ...
    public synchronized ClientKVAck handlerClientRequest(ClientKVReq request) {
        ...
        int count = 0;
        //  复制到其他机器
        for (PeerNode peer : peerSet.getPeersWithOutSelf()) {
...
//                        replicationFailQueue.offer(model);
    ...
    class ReplicationFailQueueConsumer implements Runnable {
        ...
        public void run() {
            while (running) {
                try {
                    ReplicationFailModel model = replicationFailQueue.poll(1000, TimeUnit.MILLISECONDS);
                    if (model == null) {
                        continue;
                    }
                    if (status != NodeStatus.LEADER) {
    ...
    class ElectionTask implements Runnable {
        ...
        public void run() {
            ...
            List<PeerNode> peers = peerSet.getPeersWithOutSelf();
    ...
    @Override
    public Result addPeer(PeerNode newPeer) {
        return delegate.addPeer(newPeer);
    }

    @Override
    public Result removePeer(PeerNode oldPeer) {
